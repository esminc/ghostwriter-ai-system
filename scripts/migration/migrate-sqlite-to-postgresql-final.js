...
class SQLiteToPostgreSQLMigrator {
  ...
  async dropAndRecreatePostgreSQLTables() {
    ...
    const tableDefinitions = `
      ...
      CREATE TABLE ghostwrite_history (
        ...
        calendar_data JSONB,
        slack_data JSONB,
        quality_rating DECIMAL(3,2),  -- 小数点対応（例: 4.2）
        ai_analysis_used BOOLEAN DEFAULT FALSE,
      ...
      this.sqliteDb.all(`SELECT * FROM ${tableName}`, async (err, rows) => {
        ...
        for (const row of rows) {
          const values = columns.map(col => {
            ...
            // quality_ratingの型変換（DECIMAL対応）
            if (col === 'quality_rating' && value !== null) {
  ...
  async verifyMigration() {
    ...
    for (const table of tables) {
      ...
      if (pgCount === sqliteCount) {
        console.log(`   🎯 ${table}: 100% 完全移行成功`);
      } else {
        console.log(`   ⚠️ ${table}: ${Math.round((pgCount/sqliteCount)*100)}% 移行完了`);
      }
    }
  }
  ...
  async run() {
    try {
      console.log('🚀 SQLite → PostgreSQL 移行開始（最終修正版）');
      ...
      console.log('🔧 必要な環境変数設定:');
      console.log('   export DB_TYPE=postgresql');
      console.log('   または .env ファイルで DB_TYPE=postgresql を設定');
