    ...
    async postToEsa(diary, isDryRun = true) {
        ...
        const postData = {
            post: {
                ...
                message: 'AI代筆システムによる自動投稿（WIP状態で投稿）'
            }
        };
        ...
        if (isDryRun) {
            ...
            console.log(`タグ: ${postData.post.tags.join(', ')}`);            console.log(`WIP状態: ${postData.post.wip ? '✅ 有効' : '❌ 無効'}`);
            ...
            return {
                success: true,
                dryRun: true,
                message: 'ドライラン完了 - 実際の投稿は行われていません',
                postData: postData
            };
        }
...
* **投稿者**: esa_bot (AI代筆システム)`,
            ...
            if (postResult.dryRun) {
                ...
                console.log('\n🎯 **実投稿の実行方法**:');
                console.log('node test-esa-posting.js "岡本拓也" U040L7EJC0Z --real-post');
            } else {
                console.log('\n📤 **実投稿結果**:');
                console.log('・日記生成: ✅ 成功');
...
async function runEsaPostingTest() {
    ...
    if (!userName || !slackUserId) {
        console.log('❌ 使用方法: node test-esa-posting.js [USER_NAME] [SLACK_USER_ID] [OPTIONS]');
        console.log('例: node test-esa-posting.js "岡本拓也" U040L7EJC0Z');
        console.log('オプション:');
    ...
    console.log('📝 esa投稿機能テスト開始（完全統合版）');
    console.log('='.repeat(50));
