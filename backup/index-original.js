...
const ProfileAnalyzer = require('./services/mcp-profile-analyzer');
const DiaryGenerator = require('./services/ai-diary-generator');
...
async function main() {
  try {
    ...
    // AI Diary GeneratorはprofileAnalysisを引数として取るため、適切なデータ形式に変換
    const profileAnalysis = {
      userName: 'okamoto-takuya',
      writingStyle: JSON.parse(testProfile.writing_style),
      interests: JSON.parse(testProfile.interests),
      behaviorPatterns: JSON.parse(testProfile.behavior_patterns)
    };
    ...
    // おまかせ日記生成テスト
    console.log('\n🔄 おまかせ日記生成テスト...');
    const autoResult = await diaryGenerator.generateAutomaticDiary(
      'okamoto-takuya',
      profileAnalysis,
      { contextData: 'テストコンテキスト' }
    );
